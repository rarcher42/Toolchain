...
meta_cpu_state_t meta_cpu_state;
...
{
	return meta_cpu_state.cpu_type;
}

void set_cpu_type(uint8_t ct)
{
	meta_cpu_state.cpu_type = ct;
}
...
uint32_t calc_EA(void)
{
	...
	case OP_STACK:
		if ((meta_cpu_state.cpu_type == CPU_65c02) || (meta_cpu_state.cpu_type == CPU_6502) ||
			(cpu_state.em == 1)) {
...
void print_fetchbuffer(void)
{
	...
	printf("ir=");
	for (i = 0; i < meta_cpu_state.oplen; i++) {
		printf("%02X ", meta_cpu_state.ir[i]);
	}
	printf("\n");   
}
...
void cpu_fetch(uint32_t addr)
{
	...
	op = cpu_read(addr);
	meta_cpu_state.ir[0] = op;
	meta_cpu_state.oplen = get_oplen(op);
	meta_cpu_state.addr_mode = get_addr_mode(op);
	if (meta_cpu_state.oplen > 1) {
		for (i = 1; i < meta_cpu_state.oplen; i++) {
			meta_cpu_state.ir[i] = cpu_read(addr+i);
		}
	}
	// print_fetchbuffer();
}
