...
NOB = 0x0000        # No branch
ABS = 0x8000        # Absolute branch
REL = 0x4000        # Relative branch
IND = 0x2000        # Indirect addressing
BYX = 0x100
BYY = 0x080
...
A16 = ABS | B16     # 16 bit absolute branch/jump/call
I16 = ABS | B16 | IND
A24 = ABS | B24     # 24 bit absolute branch/jump/call
R08 = REL | B08
R16 = REL | B16
AIX = ABS | B16 | IND | BYX
AIL = ABS | B24 | IND
...
("COP", (2,2,2,2,0,0), ()), # COP #10           ;$02
("ORA", (2,2,2,2,0,0), ()), # ORA 1,S           ;$03 
("TSB", (2,2,2,2,2,0), ()), # TSB $30           ;$04  
("ORA", (2,2,2,2,2,2), ()), # ORA $42           ;$05
("ASL", (2,2,2,2,2,2), ()), # ASL $22           ;$06
("ORA", (2,2,2,2,0,0), ()), # ORA [$33]         ;$07

# 08
("PHP", (1,1,1,1,1,1), ()),  # PHP              ;$08
("ORA", (3,3,2,2,2,2), ()),  # ORA #$42         ;$09
("ASL", (1,1,1,1,1,1), ()),  # ASL A            ;$0A
("PHD", (1,1,1,1,0,0), ()),  # PHD              ;$0B
("TSB", (3,3,3,3,3,0), ()),  # TSB $0300        ;$0C
("ORA", (3,3,3,3,3,3), ()),  # ORA $4242        ;$0D
("ASL", (3,3,3,3,3,3), ()),  # ASL $5150        ;$0E
("ORA", (4,4,4,4,0,0), ()),  # ORA $123124      ;$0F
# 10
("BPL", (2,2,2,2,2,2), ()),  # BPL HERE         ;$10
("ORA", (2,2,2,2,2,2), ()),  # ORA ($42),y      ;$11
("ORA", (2,2,2,2,2,0), ()),  # ORA ($42)        ;$12
("ORA", (2,2,2,2,0,0), ()),  # ORA (1,S),Y      ;$13
("TRB", (2,2,2,2,2,0), ()),  # TRB $19          ;$14
...
("BIT", (2,2,2,2,2,2), ()),  # BIT $20          ;24
("AND", (2,2,2,2,2,2), ()),  # AND $30          ;25
("ROL", (2,2,2,2,2,2), ()),  # ROL $12          ;26
("AND", (2,2,2,2,0,0), ()),  # AND [$10]        ;27
# 28
("PLP", (1,1,1,1,1,1), ()),  # PLP              ;$28
("AND", (3,3,2,2,2,2), ()),  # AND #$FE         ;$29
("ROL", (1,1,1,1,1,1), ()),  # ROL A            ;2A
("PLD", (1,1,1,1,0,0), ()),  # PLD              ;2B
("BIT", (3,3,3,3,3,3), ()),  # BIT $0BCD        ;2C
("AND", (3,3,3,3,3,3), ()),  # AND $0DEF        ;2D
("ROL", (3,3,3,3,3,3), ()),  # ROL $5150        ;2E
("AND", (4,4,4,4,0,0), ()),  # AND $123456      ;2F
#30          
...
("WDM", (2,2,2,2,0,0), ()),  # WDM #$AA         ;42
("EOR", (2,2,2,2,0,0), ()),  # EOR 1,S          ;43
("MVN", (3,3,3,3,0,0), ()),  # MVP 1,2          ;44
("EOR", (2,2,2,2,2,2), ()),  # EOR $12          ;45
("LSR", (2,2,2,2,2,2), ()),  # LSR $33          ;46 
("EOR", (2,2,2,2,0,0), ()),  # EOR [$27]        ;47
# 48
("PHA", (1,1,1,1,1,1), ()),  # PHA              ;$48
("EOR", (3,3,2,2,2,2), ()),  # EOR #$FF         ;49
("LSR", (1,1,1,1,1,1), ()),  # LSR A            ;4A
("PHK", (1,1,1,1,0,0), ()),  # PHK              ;4B
("JMP", (3,3,3,3,3,3), ()),  # JMP $2A04        ;4C
("EOR", (3,3,3,3,3,3), ()),  # EOR $ABCD        ;4D
("LSR", (3,3,3,3,3,3), ()),  # LSR $2525        ;4E
("EOR", (4,4,4,4,0,0), ()),  # EOR $123456      ;4F
# 50
...
("STZ", (2,2,2,2,2,0), ()),  # STZ $30          ;64
("ADC", (2,2,2,2,2,2), ()),  # ADC $30          ;65
("ROR", (2,2,2,2,2,2), ()),  # ROR $59          ;66
("ADC", (2,2,2,2,0,0), ()),  # ADC [$89]        ;67
# 68
("PLA", (1,1,1,1,1,1), ()),  # PLA              ;$68
("ADC", (3,3,2,2,2,2), ()),  # ADC #$23         ;69
("ROR", (1,1,1,1,1,1), ()),  # ROR A            ;6A
("RTL", (1,1,1,1,0,0), ()),  # RTL              ;6B
("JMP", (3,3,3,3,3,3), ()),  # JMP ($FFFC)      ;6C
("ADC", (3,3,3,3,3,3), ()),  # ADC $1234        ;6D
("ROR", (3,3,3,3,3,3), ()),  # ROR $2345        ;6E
("ADC", (4,4,4,4,0,0), ()),  # ADC $123456      ;6F
# 70
...
("STY", (2,2,2,2,2,2), ()),  # STY $21          ;84
("STA", (2,2,2,2,2,2), ()),  # STA $64          ;85
("STX", (2,2,2,2,2,2), ()),  # STX $99          ;86
("STA", (2,2,2,2,0,0), ()),  # STA [$55]        ;87
# 88
("DEY", (1,1,1,1,1,1), ()),  # DEY              ;88
("BIT", (3,3,2,2,2,0), ()),  # BIT #$31         ;89
("TXA", (1,1,1,1,1,1), ()),  # TXA              ;8A
("PHB", (1,1,1,1,0,0), ()),  # PHB              ;8B
("STY", (3,3,3,3,3,3), ()),  # STY $5150        ;8C
("STA", (3,3,3,3,3,3), ()),  # STA $1234        ;8D
("STX", (3,3,3,3,3,3), ()),  # STX $2345        ;8E
("STA", (4,4,4,4,0,0), ()),  # STA $123456      ;8F
# 90
...
("STZ", (3,3,3,3,3,0), ()),  # STZ $5150        ;9C
("STA", (3,3,3,3,3,3), ()),  # STA $0500,X      ;9D
("STZ", (3,3,3,3,3,0), ()),  # STZ $5555,X      ;9E
("STA", (4,4,4,4,0,0), ()),  # STA $123456,X    ;9F
# A0
("LDY", (3,2,3,2,2,2), ()),  # LDY #$44         ;$A0
("LDA", (2,2,2,2,2,2), ()),  # LDA ($27,X)      ;A1
("LDX", (3,2,3,2,2,2), ()),  # LDX #$24         ;A2
("LDA", (2,2,2,2,0,0), ()),  # LDA 5,S          ;A3
("LDY", (2,2,2,2,2,2), ()),  # LDY $55          ;A4
("LDA", (2,2,2,2,2,2), ()),  # LDA $68          ;A5
("LDX", (2,2,2,2,2,2), ()),  # LDX $88          ;A6
("LDA", (2,2,2,2,0,0), ()),  # LDA [$20]        ;A7

# A8
("TAY", (1,1,1,1,1,1), ()),  # TAY              ;$A8
("LDA", (3,3,2,2,2,2), ()),  # LDA #$AB         ;$A9
("TAX", (1,1,1,1,1,1), ()),  # TAX              ;AA
("PLB", (1,1,1,1,0,0), ()),  # PLB              ;AB
("LDY", (3,3,3,3,3,3), ()),  # LDY $1234        ;AC
("LDA", (3,3,3,3,3,3), ()),  # LDA $1234        ;AD
("LDX", (3,3,3,3,3,3), ()),  # LDX $1234        ;AE
("LDA", (4,4,4,4,0,0), ()),  # LDX $1234        ;AE
...
# C0
("CPY", (3,2,3,2,2,2), ()),  # CPY #$42         ;$C0
("CMP", (2,2,2,2,2,2), ()),  # CMP ($50,X)      ;C1
("REP", (2,2,2,2,0,0), ()),  # REP #$30         ;C2
("CMP", (2,2,2,2,0,0), ()),  # CMP 1,S          ;C3
("CPY", (2,2,2,2,2,2), ()),  # CPY $22          ;C4
("CMP", (2,2,2,2,2,2), ()),  # CMP $33          ;C5
("DEC", (2,2,2,2,2,2), ()),  # DEC $44          ;C6
("CMP", (2,2,2,2,0,0), ()),  # CMP [$42]        ;C7
# C8
("INY", (1,1,1,1,1,1), ()),  # INY              ; $C8
("CMP", (3,3,2,2,2,2), ()),  # CMP #$51         ;C9
("DEX", (1,1,1,1,1,1), ()),  # DEX              ;CA
("WAI", (1,1,1,1,1,0), ()),  # WAI              ;CB
("CPY", (3,3,3,3,3,3), ()),  # CPY $4242        ;CC
("CMP", (3,3,3,3,3,3), ()),  # CMP $4141        ;CD
("DEC", (3,3,3,3,3,3), ()),  # DEC $2525        ;CE
("CMP", (4,4,4,4,0,0), ()),  # CMP $125050      ;CF
# D0
("BNE", (2,2,2,2,2,2), ()),  # BNE T03          ;$D0
("CMP", (2,2,2,2,2,2), ()),  # CMP ($00),Y      ;D1
("CMP", (2,2,2,2,2,0), ()),  # CMP ($01)        ;D2
("CMP", (2,2,2,2,0,0), ()),  # CMP (1,S),Y      ;D3
("PEI", (2,2,2,2,0,0), ()),  # PEI $31          ;D4
...
# E0
("CPX", (3,2,3,2,2,2), ()),  # CPX #$FF         ;$E0
("SBC", (2,2,2,2,2,2), ()),  # SBC ($01,X)      ;E1
("SEP", (2,2,2,2,0,0), ()),  # SEP #$00         ;E2
("SBC", (2,2,2,2,0,0), ()),  # SBC 1,S          ;E3
("CPX", (2,2,2,2,2,2), ()),  # CPX $33          ;E4
("SBC", (2,2,2,2,2,2), ()),  # SBC $99          ;E5
("INC", (2,2,2,2,2,2), ()),  # INC $28          ;E6
("SBC", (2,2,2,2,0,0), ()),  # SBC [$42]        ;E7
# E8
("INX", (1,1,1,1,1,1), ()),  # INX              ;$E8
("SBC", (3,3,2,2,2,2), ()),  # SBC #$12         ;E9
("NOP", (1,1,1,1,1,1), ()),  # NOP              ;EA
("XBA", (1,1,1,1,0,0), ()),  # XBA              ;EB
("INC", (3,3,3,3,3,3), ()),  # CPX $3124        ;EC
("SBC", (3,3,3,3,3,3), ()),  # SBC $5101        ;ED
("INC", (3,3,3,3,3,3), ()),  # INC $2222        ;EE
("SBC", (4,4,4,4,0,0), ()),  # SBC $123456      ;EF
# F0
